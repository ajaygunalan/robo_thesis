...
## Solution 1: [[replay_buffer|Replay Buffers]]

To fix **Correlated Samples**, we use a [[replay_buffer]] (Experience Replay).
...
See [[replay_buffer]] for why this works (decorrelation + data efficiency).

---

## Solution 2: [[target_network|Target Networks]]

To fix **Moving Targets**, we introduce a [[target_network]].
...
See [[target_network]] for update strategies (hard vs soft updates).
...
## [[n_step_returns|Multi-Step Returns (N-Step)]]

See [[n_step_returns]] for the full bias-variance analysis. In DQN context:

$$y_{t} = \sum_{k=0}^{N-1} \gamma^k r_{t+k} + \gamma^N \max_{a'} Q_{\phi'}(s_{t+N}, a')$$

- **Pro:** Propagates rewards faster (less bias).
- **Con:** Technically requires on-policy data. If $N$ is small, we often ignore the off-policy correction.
...
- **Prioritized Experience Replay:** Don't sample uniformly. Sample transitions with high [[temporal_difference|TD-error]] (high "surprise") more frequently.
...
|Problem|Solution|
|---|---|
|Correlated Samples|[[replay_buffer]]|
|Moving Targets|[[target_network]]|
|Overestimation Bias|Double Q-Learning|
|Bias/Variance Tradeoff|[[n_step_returns]]|
