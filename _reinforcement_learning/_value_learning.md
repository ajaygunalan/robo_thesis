...
Instead of fitting a whole batch, we can update parameters after every step. This is [[temporal_difference]] learning applied to Q-values.
...
**Critique:** This looks like gradient descent, but strictly speaking it isn't. The target $y_i$ also depends on $\phi$ (the network parameters). We are chasing a moving target—see [[target_network]] for the fix.
...
However, in practice, we can make them work well with specific tricks (like [[replay_buffer|Replay Buffers]] and [[target_network|Target Networks]]), which forms the basis of **DQN (Deep Q-Networks)**.
