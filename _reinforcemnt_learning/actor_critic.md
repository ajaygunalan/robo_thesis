...
### Option 2: Bootstrapped Targets (TD-style)
...
**Intuition:** Instead of waiting for the full trajectory, use: "reward now + my current estimate of future value."
...
**Question:** Can we reuse old transitions from a replay buffer?

---

## The "Broken" Naive Off-Policy Algorithm
Before learning the solution, it is important to understand why we cannot simply use a Replay Buffer with the standard Actor-Critic logic (Slides 21-23).
...
Store transitions $(s, a, r, s')$ in a replay buffer. For each sampled batch:
...
- There are many **fancier ways to fit $Q_\phi$** than the naïve regression above (target networks, double Q-learning), which is why these ideas show up in modern algorithms like Soft Actor–Critic.
...
## Solution: N-Step Returns and GAE
...
**Intuition:** Use real rewards for near-term (low variance there), cut off with value function before variance explodes in the far future. In practice, choosing an intermediate $n>1$ often works better than either extreme. These n-step schemes are closely related to the classical notion of **eligibility traces**.

### Generalized Advantage Estimation (GAE)

Instead of picking one $n$, take a weighted average of _all_ n-step returns.

Define TD residual: $\delta_t = r_t + \gamma V(s_{t+1}) - V(s_t)$

**GAE formula:** $$ \hat{A}_t^{\text{GAE}(\gamma,\lambda)} = \sum_{l=0}^{\infty} (\gamma \lambda)^l \delta_{t+l} $$

**Implementation Note (Recursive Form):** Instead of summing to infinity, GAE is usually computed backwards in time: $$\hat{A}_t^{\text{GAE}} = \delta_t + (\gamma \lambda) \hat{A}_{t+1}^{\text{GAE}}$$

- $\lambda = 0$: Only use $\delta_t$ (pure 1-step TD)
- $\lambda = 1$: Approaches Monte-Carlo
- $\lambda \in (0.9, 0.99)$: Practical sweet spot

Both $\gamma$ and $\lambda$ control bias-variance tradeoff in complementary ways.
...
|**Data usage**|On-policy vs off-policy + replay buffer|
...
|On-policy wastes data|Off-policy with Q-function + replay buffer|
|Want unbiased gradients|Use critic as baseline only|
|TD vs MC is a hard choice|N-step returns or GAE|
